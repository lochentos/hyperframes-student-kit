...
output-timeline offsets, applies 2-word UPPERCASE chunks.
...
_spec = importlib.util.spec_from_file_location("grade", _here / "grade.py")
_grade_mod = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_grade_mod)
get_preset = _grade_mod.get_preset
auto_grade_for_clip = _grade_mod.auto_grade_for_clip
...
def extract_all_segments(edl: dict, edit_dir: Path, preview: bool, draft: bool = False) -> list[Path]:
    ...
    for i, r in enumerate(ranges):
        ...
        seg_filter = auto_grade_for_clip(src_path, start=start, duration=duration)[0] if is_auto else resolved
...
def apply_loudnorm_two_pass(input_path: Path, output_path: Path, preview: bool = False) -> bool:
    ...
    if measurement is None:
        print("  measurement failed — falling back to 1-pass")
        return apply_loudnorm_two_pass(input_path, output_path, preview=True)

    print(f"    measured: I={measurement['input_i']} LUFS  TP={measurement['input_tp']}  LRA={measurement['input_lra']}")
...
def build_final_composite(
    ...
    for idx, ov in enumerate(overlays, start=1):
        ...
        next_label = f"[v{idx}]"
        filter_parts.append(f"{current}[a{idx}]overlay=enable='between(t,{t:.3f},{end:.3f})'{next_label}")
        current = next_label

    if has_subs:
        subs_abs = str(subtitles_path.resolve()).replace("\\", "/").replace(":", r"\:")
        filter_parts.append(f"{current}subtitles='{subs_abs}':force_style='{SUB_FORCE_STYLE}'[outv]")
        out_label = "[outv]"
    else:
        if has_overlays:
...
def main() -> None:
    ...
    ap.add_argument("--preview", action="store_true",
                    help="Preview: 1080p, medium, CRF 22")
    ap.add_argument("--draft", action="store_true",
                    help="Draft: 720p, ultrafast, CRF 28")
    ap.add_argument("--build-subtitles", action="store_true",
                    help="Build master.srt from transcripts + EDL offsets")
    ap.add_argument("--no-subtitles", action="store_true",
                    help="Skip subtitles even if EDL references one")
